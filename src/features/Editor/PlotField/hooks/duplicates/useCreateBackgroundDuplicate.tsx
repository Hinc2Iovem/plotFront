...
type CreateBackgroundDuplicateOnMutation = {
  commandOrder: number;
  commandIfId?: string;
  isElse?: boolean;
  topologyBlockId: string;
...
export default function useCreateBackgroundDuplicate({ topologyBlockId, episodeId }: CreateBackgroundDuplicateTypes) {
  ...
  return useMutation({
    ...
    }: CreateBackgroundDuplicateOnMutation) => {
      const currentTopologyBlockId = bodyTopologyBlockId?.trim().length ? bodyTopologyBlockId : topologyBlockId;

      await axiosCustomized
        .post(`/plotFieldCommands/backgrounds/topologyBlocks/${currentTopologyBlockId}/copy`, {
          commandOrder,
          plotfieldCommandId,
          commandIfId,
          isElse,
    ...
    onMutate: async (newCommand: CreateBackgroundDuplicateOnMutation) => {
      addItem({
        episodeId,
        item: {
