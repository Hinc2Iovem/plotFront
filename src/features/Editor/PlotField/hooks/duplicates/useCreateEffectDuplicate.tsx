...
type CreateEffectDuplicateOnMutation = {
  commandOrder: number;
  commandIfId?: string;
  isElse?: boolean;
  topologyBlockId: string;
...
export default function useCreateEffectDuplicate({
  topologyBlockId,
  episodeId,
  ...
  return useMutation({
    ...
    }: CreateEffectDuplicateOnMutation) => {
      const currentTopologyBlockId = bodyTopologyBlockId?.trim().length
        ? bodyTopologyBlockId
        : topologyBlockId;
      await axiosCustomized
        .post(
          `/plotFieldCommands/effects/topologyBlocks/${currentTopologyBlockId}/copy`,
          {
            commandOrder,
            plotfieldCommandId,
            commandIfId,
    ...
    onMutate: async (newCommand: CreateEffectDuplicateOnMutation) => {
