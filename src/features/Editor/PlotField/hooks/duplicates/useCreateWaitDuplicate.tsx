...
type CreateWaitDuplicateOnMutation = {
  commandOrder: number;
  commandIfId?: string;
  isElse?: boolean;
  topologyBlockId: string;
...
export default function useCreateWaitDuplicate({ topologyBlockId, episodeId }: CreateWaitDuplicateTypes) {
  ...
  return useMutation({
    ...
    onMutate: async (newCommand: CreateWaitDuplicateOnMutation) => {
      addItem({
        episodeId,
        item: {
