...
type CreateCutSceneDuplicateOnMutation = {
  commandOrder: number;
  commandIfId?: string;
  isElse?: boolean;
  topologyBlockId: string;
...
export default function useCreateCutSceneDuplicate({ topologyBlockId, episodeId }: CreateCutSceneDuplicateTypes) {
  ...
  return useMutation({
    ...
    }: CreateCutSceneDuplicateOnMutation) => {
      const currentTopologyBlockId = bodyTopologyBlockId?.trim().length ? bodyTopologyBlockId : topologyBlockId;
      await axiosCustomized
        .post(`/plotFieldCommands/cutScenes/topologyBlocks/${currentTopologyBlockId}/copy`, {
          commandOrder,
          plotfieldCommandId,
          commandIfId,
          isElse,
