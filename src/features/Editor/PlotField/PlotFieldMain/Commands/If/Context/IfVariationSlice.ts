...
export type IfVariationSliceTypes = {
  ifs: {
...
export const createIfVariationSlice: StateCreator<IfVariationSliceTypes, [], [], IfVariationSliceTypes> = (
  set,
  get
) => ({
  ifs: [],
              ...
              ifVariations: c.ifVariations.map((cov) =>
                cov.ifVariationId === ifVariationId
                  ? {
                      ...
                      currentlyDressed: typeof currentlyDressed === "boolean" ? currentlyDressed : cov.currentlyDressed,
                    }
                  : cov
              ),
            }
  ...
  updateLogicalOperator: ({ index, logicalOperator, plotfieldCommandId }) => {
    ...
    if (currentLogicalOperators?.trim()?.length) {
      ...
      if (typeof index === "number" && index >= 0 && index <= (splittedLogicalOperators?.length || 0) - 1) {
        ...
        set((state: IfVariationSliceTypes) => {
      ...
      ifs: state.ifs.map((c) =>
        c.plotfieldCommandId === plotfieldCommandId
          ? {
              ...c,
              logicalOperators:
                c.logicalOperators?.trim()?.length > 0 ? `${c.logicalOperators},${logicalOperator}` : logicalOperator,
            }
          : c
      ),
    })),
