...
const useResizeEditorWindow = ({ containerRef, command, setScaleDivPosition }: ResizeEditorWindowTypes) => {
  useEffect(() => {
    const resizeEditorWindow = () => {
      if (containerRef.current) {
        if (command === "expandFlowchart") {
          setScaleDivPosition(window.innerWidth / 2 - containerRef.current.clientWidth / 2 + 10);
        } else {
