...
export function GoingDownInsideChoice({
  ...
  if (isGoingDown) {
    ...
    if (!firstOption) {
      console.log("You haven't created any option, or at least you haven't assigned any topology block to them, yet.");
      return;
    }
    ...
    sessionStorage.setItem("focusedTopologyBlock", `${firstOption.topologyBlockId}`);
  }
}
